  ...
  if (!courses.length) {
    ...
    return (
      <div className={boardStyle.paymentBoard}>
        ...
        <div>
          {courses.some((course: any) => course.status === "Active") ? <div className={boardStyle.payCardList}> {courses.map((course: any, index: any) => {
            return <div key={index}>
              <PaymentCard head={course.title} subhead={course.subscriptionName} total={course.amount} availableSlot={course.noOfSlotUsed} totalSlot={course.noOfSeats}/>
            </div>
          })}
          </div>:<div className={boardStyle.centerEmptyState}>
            < EmptyState imag={emptStateBio} text="<button className='smallButton'>subscribe</button>" head='<h5 className="mediumText">You do not have an active subscription</h5>' />
          </div>
          }
        </div>
