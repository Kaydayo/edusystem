...
const CompanyForm = () => {
  ...
  const addedCompanyValues = useSelector((state:RootState)=> state.companyonboard.info.values)
  ...
  return (
    <div className={companyStyle.admin}>
      ...
      <div className='fieldInput'>
        ...
        {
          addedCompanyValues && addedCompanyValues.map((value: string, index: any) => {
            return (
              <div className={companyStyle.values} key={index}>
                <input value={value} onChange={(e) => {
                  const updateValue = addedCompanyValues.map((item:string, idx:any) => {
                    if (idx === index) {
                  ...
                  if (e.target.value ==="") {
                    const newList = addedCompanyValues.filter((currValue: string) => currValue !== value)
                ...
                <button className={companyStyle.removeBtn} onClick={() => {
                  const newList = addedCompanyValues.filter((currValue: string) => currValue !== value)
                  dispatch(addValuesList(newList))
                }}>Remove</button>
              </div>
        ...
        <div className={companyStyle.values}>
          <input  value={currValue} onChange={(e)=>setCurrValue(e.target.value)}/>
          <button onClick={() => {
            if (currValue !== "") {
